...
export const getUserEventsController = async (req, res) => {
  ...
  try {
    ...
    const userUpcomingEvents = eventWithStats.filter(
      (e) => e.status === "upcoming",
    );
    const userOngoingEvents = eventWithStats.filter(
      (e) => e.status === "ongoing",
    );
    const userPastEvents = eventWithStats.filter((e) => e.status === "past");
    return res
      .status(200)
      .json({
        success: true,
        ...
        ongoingEvents: userOngoingEvents,
